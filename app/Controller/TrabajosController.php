...
class TrabajosController extends AppController {
    ...
    public function creditos() {
        if ($this->RequestHandler->responseType() == 'json') {
            $pagar = '<button class="btn btn-success btn-xs" type="button" onclick="pagar(' . "',Nota.id,'" . ')"><i class="icon ico-coins"></i>Pagar</button>';
            $sql1 = "SELECT nombre FROM `clientes` WHERE (clientes.id = Trabajo.cliente_id)";
            $check_aux = "',Nota.id,'";
            $check = '<input type="checkbox" name="data[notas][' . $check_aux . ']" class="form-control">';
            $this->Nota->virtualFields = array(
                'check' => "CONCAT('$check')",
                'cliente' => "CONCAT(($sql1))",
                'fecha_nota' => "(DATE(Nota.created))",
                'accion' => "CONCAT('$pagar')"
            );
            $this->paginate = array(
                'fields' => array('Nota.check', 'Nota.fecha_nota', 'Nota.numero', 'Nota.trabajo_id', 'Nota.cliente', 'Sucursale.nombre', 'Nota.tipo', 'Nota.numero_factura', 'Nota.tipo_pago', 'Nota.total_pagado', 'Nota.accion'),
                'recursive' => 0,
                'order' => 'Nota.created DESC',
                'conditions' => array('Nota.tipo_pago' => 'Credito')
            );
            $this->DataTable->fields = array('Nota.check', 'Nota.fecha_nota', 'Nota.numero', 'Nota.trabajo_id', 'Nota.cliente', 'Sucursale.nombre', 'Nota.tipo', 'Nota.numero_factura', 'Nota.tipo_pago', 'Nota.total_pagado', 'Nota.accion');
            $this->DataTable->emptyEleget_usuarios_adminments = 1;
            $this->set('notas', $this->DataTable->getResponse('Trabajos', 'Nota'));
            $this->set('_serialize', 'notas');
        }
    }

    public function pagar_credito($idNota = null) {
        $this->Nota->id = $idNota;
        $d_nota['tipo_pago'] = 'Credito Pagado';
        $this->Nota->save($d_nota);
        $this->Session->setFlash("La nota se ha cambiado como pagado", 'msgbueno');
    ...
    public function eliminar($idTrabajo = null) {
        ...
        $this->Hojasproduccione->deleteAll(array('Hojasproduccione.trabajo_id' => $idTrabajo));
        $this->Hojastipostrabajo->deleteAll(array('Hojastipostrabajo.trabajo_id' => $idTrabajo));
        $this->Nota->deleteAll(array('Nota.trabajo_id' => $idTrabajo));
